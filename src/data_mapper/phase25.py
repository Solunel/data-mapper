"""Phase 2.5 P0：只读导出与人工 Gold 审核门禁。"""
...
def build_gold_review_draft(
    ...
    *,
    ...
    if not plan_items:
        raise Phase25ReviewError("至少需要一个 Phase 2 MappingPlan")
    if context_window < 0:
    ...
    for plan in sorted(plan_items, key=_plan_sort_key):
        if plan.ontology_catalog.ontology_revision != catalog.ontology_revision:
            raise Phase25ReviewError(
                "MappingPlan 与 OntologyCatalog 的 ontology_revision 不一致："
                f"{plan.mapping_run_id}"
            )
        assignment = assignments.get(plan.mapping_run_id)
    ...
    if not cases:
        raise Phase25ReviewError("Phase 2 MappingPlan 中没有 eligible 未决 Metric")
    ...
    mapping_run_ids = tuple(sorted(plan.mapping_run_id for plan in plan_items))
    draft_id = _stable_id(
        "gold-review-draft",
        {
...
def _plan_sort_key(plan: MappingPlan) -> tuple[str, str, str]:
    return (
        plan.table_mapping_plan.source_file,
        plan.table_mapping_plan.sheet_name,
        plan.mapping_run_id,
    )
