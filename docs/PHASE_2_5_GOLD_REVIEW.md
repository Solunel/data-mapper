...
当前状态：**首版 Gold Truth 已确认，P1 准入门禁通过。**
...
- `利息费用 → qc.interest_expense`：利润表上下文与本体定义共同支持同一利息
  费用核算对象；`qc.finance_expenses` 是更宽口径困难负样本；
...
顶层 revision、`draft_id`、`mapping_run_ids`、`ontology_metrics`，以及每个案例的
